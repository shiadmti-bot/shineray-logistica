...
import { 
    ...
    TruckIcon, ScissorsIcon, CameraIcon, HandThumbUpIcon, ArrowUpOnSquareIcon,
    ArrowDownOnSquareIcon, ArrowsRightLeftIcon, BuildingOffice2Icon,
    ChatBubbleBottomCenterTextIcon, PlayIcon, StopIcon
...
export default function PedidoShow({ auth, pedido }) {
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6 px-4">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        ...
                        <div className="p-6 bg-slate-50/50">
                            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                <ArrowUpOnSquareIcon className="w-4 h-4 text-blue-500" /> Origem (Sai De)
                            </h3>
                            ...
                            {pedido.previsao_coleta && (
                                <div className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-orange-700 bg-orange-50 px-3 py-1.5 rounded-full border border-orange-100">
                                    <CalendarIcon className="w-4 h-4" /> Previsão Coleta: {new Date(pedido.previsao_coleta).toLocaleDateString()}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-6">
                            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                <ArrowDownOnSquareIcon className="w-4 h-4 text-green-500" /> Destino (Vai Para)
                            </h3>
                    ...
                    {pedido.status === 'aguardando_coleta' && (
                        <div className="bg-orange-50 border-l-4 border-orange-500 p-5 rounded-r-lg shadow-sm flex items-start gap-4">
                            <div className="bg-orange-100 p-2 rounded-full text-orange-600"><ExclamationTriangleIcon className="w-6 h-6" /></div>
                            <div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                        <div className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 flex justify-between items-center backdrop-blur-sm">
                            <h3 className="font-black text-gray-800 text-sm uppercase tracking-wide flex items-center gap-2">
                                <span className="text-gray-500"><ChatBubbleBottomCenterTextIcon className="w-5 h-5"/></span> Motocicletas
                            </h3>
                    ...
                    {pedido.status !== 'cancelado' && pedido.status !== 'concluido' && (
                        <div className="bg-white p-6 shadow-lg rounded-2xl border-l-4 border-blue-600 mt-8">
                            <h3 className="font-bold text-lg mb-4 text-gray-800 flex items-center gap-2">
                                <span className="bg-blue-100 text-blue-600 p-1.5 rounded-lg"><BoltIcon className="w-5 h-5"/></span> Ações
                            </h3>
                            <div className="flex flex-wrap gap-4">
                                {pedido.status === 'solicitado' && (souOrigem || (souCD && !isTransferencia) || souAdmin) && (
                                    <>
                                        <button onClick={confirmarSeparacao} className="flex-1 flex justify-center items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-xl font-bold hover:from-blue-700 hover:to-blue-800 shadow-md transition"><CheckCircleIcon className="w-5 h-5"/> Confirmar Separação</button>
                                        <button onClick={handleRejeitar} className="flex-1 flex justify-center items-center gap-2 bg-white border-2 border-red-100 text-red-600 px-6 py-3 rounded-xl font-bold hover:bg-red-50 hover:border-red-200 transition"><XCircleIcon className="w-5 h-5"/> Rejeitar</button>
                                    </>
                                )}
                                {pedido.status === 'expedido' && (souCD || souAdmin) && (
                                    <button onClick={confirmarSaida} className="flex-1 flex justify-center items-center gap-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-3 rounded-xl font-bold hover:from-orange-600 hover:to-orange-700 shadow-md transition"><TruckIcon className="w-5 h-5"/> Confirmar Saída Manual</button>
                                )}
                            </div>
                        </div>
                    )}
            ...
            {/* FAB */}
            {['em_transito', 'em_transito_cd'].includes(pedido.status) && (souDestino || souCD) && (
                <div className="fixed bottom-6 left-0 right-0 flex justify-center px-4 z-50 pointer-events-none">
                    <button onClick={handleConferenciaEntrega} disabled={compressing} className="pointer-events-auto bg-gray-900 hover:bg-black text-white font-bold py-4 px-8 rounded-full shadow-2xl transition transform hover:-translate-y-1 hover:scale-105 flex items-center gap-3 border-4 border-white/20 backdrop-blur-md">
                        {compressing ? <span className="animate-spin"><ArrowPathIcon className="w-5 h-5"/></span> : <DocumentTextIcon className="w-6 h-6"/>} <span className="uppercase tracking-wide text-sm">Conferir e Finalizar Entrega</span>
                    </button>
                </div>
            )}
...
function TipoBadge({ isTransferencia }) {
    return isTransferencia 
        ? <span className="text-[10px] bg-orange-100 text-orange-800 px-2 py-1 rounded border border-orange-200 font-bold uppercase tracking-wide flex items-center gap-1"><ArrowsRightLeftIcon className="w-3 h-3"/> Transferência</span>
        : <span className="text-[10px] bg-blue-100 text-blue-800 px-2 py-1 rounded border border-blue-200 font-bold uppercase tracking-wide flex items-center gap-1"><BuildingOffice2Icon className="w-3 h-3"/> Reposição CD</span>;
}
...
function Timeline({ status, isTransferencia }) {
    let steps = [];
    if (isTransferencia) {
        steps = [{ id: 'solicitado', label: 'Aprovado' }, { id: 'separado', label: 'Separado' }, { id: 'aguardando_coleta', label: 'Aguard. Coleta' }, { id: 'em_transito', label: 'Em Trânsito' }, { id: 'concluido', label: 'Entregue' }];
    } else {
        steps = [{ id: 'solicitado', label: 'Solicitado' }, { id: 'separado', label: 'Separado' }, { id: 'expedido', label: 'Expedido' }, { id: 'em_transito', label: 'Em Trânsito' }, { id: 'concluido', label: 'Entregue' }];
    }
