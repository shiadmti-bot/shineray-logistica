    ...
    return (
        <AppLayout user={auth.user}>
            ...
            {/* NAVEGAÇÃO SUPERIOR */}
                        ...
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`px-4 py-3 rounded-xl transition-all duration-300 shadow-sm flex flex-col md:flex-row items-center justify-center md:justify-start gap-2 border-2 relative overflow-hidden group
                                    ${activeTab === tab.id 
                                        ? `bg-surface-card border-${tab.color}-500 text-${tab.color}-700 shadow-md ring-2 ring-${tab.color}-100 transform -translate-y-1` 
                                        : 'bg-surface-card text-content-muted border-transparent hover:bg-surface-sunken hover:border-line'
                                    }`}
                            >
                    ...
                    <div className="bg-surface-card overflow-hidden shadow-xl rounded-2xl border-t-4 border-line-strong animate-fade-in-up min-h-[600px]">
                        <div className="p-8 md:p-12">
                            ...
                            {activeTab === 'loja' && (
                                <div className="space-y-10 animate-fade-in">
                                    ...
                                    <div className="bg-surface-card p-6 rounded-2xl border border-line shadow-sm space-y-8">
                                        <h3 className="text-xl font-black text-content-primary border-b pb-4">🔄 Ciclo Completo de uma Moto (Como Pedir e Receber)</h3>
                                        ...
                                        <Step number="1" title="Criando o Pedido">
                                            ...
                                            <div className="mt-3 bg-status-danger-bg border border-status-danger-solid/30 rounded-lg p-3">
                                                <p className="text-sm text-status-danger-fg font-bold">⚠️ OBRIGATÓRIO: O campo Chassi agora é obrigatório para TODOS os tipos de pedido (Reposição, Transferência e Devolução). O sistema não permitirá salvar sem o número do chassi preenchido (mínimo 11 caracteres).</p>
                                            </div>
                                        </Step>

                                        <Step number="2" title="Aguardando a Aprovação (Gestor)">
                                            <p>Ao salvar, seu pedido entra com o status <span className="text-brand-600 font-bold bg-brand-50 px-2 py-0.5 rounded">Em Análise</span>. <br/>
                                            ...
                                            <p className="mt-2 text-sm text-content-muted italic">Você não precisa fazer nada além de aguardar.</p>
                                        </Step>

                                        <Step number="3" title="Logística e Separação">
                                            <p>Quando aprovado, o status muda para <span className="text-status-success-fg font-bold bg-status-success-bg px-2 py-0.5 rounded">Solicitado</span>.</p>
                                            <ul className="list-disc ml-6 mt-3 space-y-2 text-sm text-content-secondary">
                                            ...
                                            <p className="mt-2 text-sm font-semibold text-content-secondary">Logo após a separação, a equipe de logística encaixará sua moto na próxima rota de caminhão disponível.</p>
                                        </Step>
                                        ...
                                        <Step number="4" title="Agendamento e Confirmação de Rota">
                                            ...
                                            <ul className="list-disc ml-6 mt-3 space-y-2 text-sm text-content-secondary">
                                                <li><span className="inline-block w-3 h-3 rounded-full bg-status-warning-solid mr-1"></span> <strong>Agendado (Amarelo):</strong> O CD reservou uma data provisória para a sua rota. Seu pedido <strong>NÃO</strong> muda de status ainda, apenas recebe uma previsão de entrega. A rota pode mudar.</li>
                                                <li><span className="inline-block w-3 h-3 rounded-full bg-status-success-solid mr-1"></span> <strong>Confirmado (Verde):</strong> O CD confirmou oficialmente a viagem. SÓ agora o pedido muda para <span className="text-status-success-fg font-bold bg-status-success-bg px-2 py-0.5 rounded text-xs">Rota Confirmada</span> e a data de chegada é garantida.</li>
                                            </ul>
                                            <div className="mt-3 bg-status-warning-bg border border-status-warning-solid/30 rounded-lg p-3">
                                                <p className="text-sm text-status-warning-fg">🕐 <strong>Expiração Automática:</strong> Se o dia da rota confirmada chegar e o caminhão NÃO sair, o sistema automaticamente desfaz a confirmação e retorna o pedido para a fila de espera ("Aguardando Rota" ou "Separado"). Você será notificado caso isso aconteça.</p>
                                            </div>
                                        </Step>

                                        <Step number="5" title="Coleta e Trânsito">
                                            <ul className="list-disc ml-6 space-y-2 text-sm text-content-secondary">
                                                <li>Se for transferência, o motorista vai até a loja de origem. Lá, o motorista bipa a carga e o sistema muda o status para <span className="text-status-success-fg font-bold bg-status-success-bg px-2 py-0.5 rounded text-xs">Coletado</span> automaticamente. Nenhuma loja precisa apertar botão!</li>
                                                <li>Assim que o caminhão for carregado e o CD liberar a saída, seu pedido aparecerá como <span className="text-status-warning-fg font-bold border border-status-warning-solid shadow-sm px-2 py-0.5 rounded text-xs"><TruckIcon className="w-3 h-3 inline" /> Em Trânsito</span> no seu Painel.</li>
                                            </ul>
                                        </Step>

                                        <Step number="6" title="Recebimento Completo (Finalização e Foto)">
                                            <p className="font-bold text-status-danger-fg mb-2">Atenção: Esta é a etapa mais importante para a segurança jurídica e financeira da loja!</p>
                                            <p>Quando o caminhão chegar na frente da sua loja com as motos:</p>
                                            <ol className="list-decimal ml-6 mt-3 space-y-3 text-sm text-content-secondary">
                                            ...
                                            <div className="mt-4 bg-status-danger-bg border border-status-danger-solid/30 rounded-lg p-3 space-y-2">
                                                <p className="text-sm text-status-danger-fg font-bold">🔒 TRAVA DE RECEBIMENTO PARCIAL:</p>
                                                <p className="text-sm text-status-danger-fg">Se o seu pedido tem 4 motos, mas o CD só enviou 2 neste caminhão (entrega parcial), o sistema <strong>NÃO</strong> vai permitir que você clique em "Finalizar". Você terá que aguardar o CD despachar as motos restantes. Só quando 100% da carga do pedido for enviada é que a finalização será liberada.</p>
                                            </div>
                                        </Step>

                                        {/* ALERTA CRUCIAL: BLOQUEIO EM TRÂNSITO */}
                                        <div className="bg-status-warning-bg border-2 border-status-warning-solid/40 rounded-xl p-5 mt-4">
                                            <h4 className="text-lg font-black text-status-warning-fg flex items-center gap-2 mb-3">
                                                <ExclamationTriangleIcon className="w-6 h-6" /> REGRA IMPORTANTE: Bloqueio de Novo Pedido
                                            </h4>
                                            <p className="text-sm text-status-warning-fg">Por determinação da gestão, se a sua loja possuir <strong>qualquer pedido</strong> com status <span className="font-bold text-status-warning-fg">"Em Trânsito"</span> (ou seja, motos a caminho que ainda não foram finalizadas/recebidas no sistema), você estará <strong>BLOQUEADO</strong> de criar novos pedidos, transferências ou devoluções.</p>
                                            <p className="text-sm text-status-warning-fg mt-2 font-bold">👉 Solução: Abra os pedidos com status "Em Trânsito", tire a foto do comprovante do motorista e finalize a entrega. Só assim o sistema liberará novas solicitações.</p>
                                        </div>
                                    </div>

                                    {/* CENÁRIO B: ENVIAR MOTO (TRANSFERÊNCIA PASSIVA) */}
                                    <div className="bg-status-warning-bg p-6 rounded-2xl border border-status-warning-solid/20 mt-8 mb-8">
                                        <h3 className="text-xl font-black text-status-warning-fg mb-4 flex items-center gap-2">
                                            <ArrowPathIcon className="w-6 h-6" /> Como Enviar Motos (Saída / Transferência)
                                        </h3>
                                        <p className="text-sm text-status-warning-fg mb-6 flex items-center gap-2">
                                        ...
                                        <Step number="1" title="Ação Urgente: Separar no Pátio">
                                            <p>Ao ver o alerta <span className="text-status-danger-fg font-bold bg-surface-card px-2 py-0.5 rounded border border-status-danger-solid/30">Ação Necessária: X Motos Pendentes de Separação</span> no painel inicial:</p>
                                        ...
                                        <Step number="2" title="Logística (Agendamento e Milk Run)">
                                            <p>A partir do momento em que a moto foi separada, o pedido fica <span className="text-brand-600 font-bold bg-status-warning-bg px-2 py-0.5 rounded">Aguardando Rota</span>.</p>
                                            <ul className="list-disc ml-6 mt-2 space-y-2 text-sm text-content-secondary">
                                                <li>O CD central vai gerar um agendamento no Calendário. Se a rota for <span className="inline-block w-3 h-3 rounded-full bg-status-warning-solid mr-1"></span><strong>Amarela (Pré-Agendada)</strong>, o pedido continua aguardando. Quando o CD confirmar a rota como <span className="inline-block w-3 h-3 rounded-full bg-status-success-solid mr-1"></span><strong>Verde</strong>, aí sim o status muda para <span className="text-status-success-fg font-bold bg-status-success-bg px-2 py-0.5 rounded">Rota Confirmada</span>.</li>
                                                <li>O status evolui para <strong className="text-status-warning-fg">Aguardando Coleta</strong> quando a viagem da semana for gerada no Romaneio.</li>
                                        ...
                                        <Step number="3" title="Baixa Automática">
                                            <p className="text-sm bg-surface-card p-3 rounded border border-status-warning-solid/30 flex items-start gap-2">
                                                <InformationCircleIcon className="w-5 h-5 text-status-warning-fg mt-0.5 flex-shrink-0" />
                                    ...
                                    {/* COMO DEVOLVER MOTO */}
                                    <div className="bg-status-danger-bg p-6 rounded-2xl border border-status-danger-solid/20">
                                        <h3 className="text-xl font-black text-status-danger-fg mb-4 flex items-center gap-2">
                                            <ArrowUturnLeftIcon className="w-6 h-6" /> Como Devolver Motos (Logística Reversa)
                                        </h3>
                                        <p className="text-sm text-status-danger-fg mb-4">Caso precise devolver uma moto para o CD (Matriz) por renegociação, troca, defeito de fábrica ou sucata, o fluxo é o mesmo de pedir uma moto, só que ao contrário.</p>
                                        <ol className="list-decimal ml-6 space-y-2 text-sm text-content-secondary">
                            ...
                            {activeTab === 'gestor' && (
                                <div className="space-y-10 animate-fade-in">
                                    ...
                                    <div className="bg-surface-card p-6 rounded-2xl border border-line shadow-sm space-y-8">
                                        <h3 className="text-xl font-black text-content-primary border-b pb-4">🛡️ O Fluxo de Aprovação Comercial</h3>
                                        ...
                                        <Step number="1" title="Notificação de Novo Pedido">
                                            ...
                                            <p className="text-sm mt-2 text-content-secondary">Acesse a aba <strong>Gestão</strong> no menu esquerdo para ver todos os pedidos aguardando o seu carimbo de liberação.</p>
                                        </Step>
                                        ...
                                        <Step number="2" title="Análise Financeira e de Cadastro">
                                            ...
                                            <p className="text-sm mt-2 font-medium text-content-primary">Verifique nos seus controles financeiros (ERP/Planilhas) se a loja possui limite de crédito ou pagou pelo produto.</p>
                                        </Step>
                                        ...
                                        <Step number="3" title="Aprovação (Total ou Parcial)">
                                            ...
                                            <div className="bg-surface-sunken p-4 mt-3 rounded-lg border border-line">
                                                <p className="text-sm mb-2 font-bold text-content-secondary">Como fazer cortes de reprovação:</p>
                                                <ul className="text-sm text-content-secondary space-y-2">
                                                    <li className="flex items-center gap-2"><CheckCircleIcon className="w-5 h-5 text-status-success-fg" /> Ao lado de cada moto listada existe um ícone Verde. Isso significa que essa unidade passará.</li>
                                                    <li className="flex items-center gap-2"><XCircleIcon className="w-5 h-5 text-status-danger-fg" /> Ao clicar no verde, ele fica Vermelho. As motos vermelhas <strong>serão sumariamente deletadas</strong> do pedido na hora da aprovação final.</li>
                                                </ul>
                                            </div>
                                        ...
                                        <Step number="4" title="Faturamento vs Logística (Importante)">
                                            <p className="text-status-danger-fg font-bold mb-1">Aprovar no Sistema Web não é a mesma coisa que faturar no ERP!</p>
                                            <p className="text-sm text-content-secondary">Quando a Diretoria aperta o botão "Aprovar":</p>
                                            <ul className="list-disc ml-6 text-sm text-content-secondary mt-2 space-y-1">
                            ...
                            {activeTab === 'cd' && (
                                <div className="space-y-10 animate-fade-in">
                                    ...
                                    <div className="bg-surface-card p-6 rounded-2xl border border-line shadow-sm space-y-8">
                                        <h3 className="text-xl font-black text-content-primary border-b pb-4">🚚 O Fluxo Físico da Moto (do Galpão ao Caminhão)</h3>
                                        ...
                                        <Step number="1" title="Motos 'No Radar' (Solicitadas)">
                                            ...
                                            <ul className="list-disc ml-6 mt-2 text-sm text-content-secondary space-y-2">
                                        ...
                                        <Step number="2" title="Agendamento de Rota no Calendário">
                                            ...
                                            <div className="mt-3 bg-status-info-bg border border-status-info-solid/30 rounded-lg p-4 space-y-3">
                                                <p className="text-sm text-status-info-fg font-bold">🟡 Agendado (Amarelo):</p>
                                                <p className="text-sm text-status-info-fg">Pré-reserva de data. Os pedidos recebem a previsão de entrega como informativo, mas <strong>NÃO</strong> mudam de status. A rota ainda pode ser cancelada ou remarcada.</p>
                                                <p className="text-sm text-status-info-fg font-bold">🟢 Confirmado (Verde):</p>
                                                <p className="text-sm text-status-info-fg">Confirmação oficial. Os pedidos daquela rota mudam automaticamente para <span className="text-status-success-fg font-bold bg-status-success-bg px-2 py-0.5 rounded text-xs">Rota Confirmada</span> e a loja já visualiza a previsão do dia de chegada.</p>
                                            </div>
                                            <div className="mt-3 bg-status-warning-bg border border-status-warning-solid/30 rounded-lg p-3">
                                                <p className="text-sm text-status-warning-fg">⏰ <strong>Rotas Expiradas:</strong> Se o dia da rota confirmada chegar e o caminhão não sair, o sistema AUTOMATICAMENTE regride os pedidos para a fila de espera e registra o motivo no log. Nenhuma ação manual é necessária.</p>
                                            </div>
                                        </Step>
                                        ...
                                        <Step number="3" title="Montando o Caminhão (Criando Romaneio)">
                                            ...
                                            <p className="mt-2 text-sm text-content-secondary">A tela mostrará a lista de tudo que está "Separado" no Pará inteiro naquele dia.</p>
                                            <ol className="list-decimal ml-6 mt-2 text-sm text-content-secondary space-y-2">
                                            ...
                                            <div className="mt-3 bg-status-success-bg border border-status-success-solid/30 rounded-lg p-3">
                                                <p className="text-sm text-status-success-fg">🔧 <strong>Entregas Parciais:</strong> Agora é possível enviar PARTE de um pedido num caminhão e a outra parte depois. O romaneio trabalha por moto individual, não por pedido inteiro. Se um pedido tem 4 motos e só 2 cabem no caminhão de hoje, selecione apenas essas 2. As motos restantes continuarão na fila para a próxima viagem.</p>
                                            </div>
                                        </Step>

                                        <Step number="4" title="O Caminhão Caiu na Estrada">
                                            <p>Após imprimir os papéis (manifestos), clique no botão <span className="text-status-warning-fg font-bold border border-status-warning-solid px-2 py-0.5 rounded text-xs">Aprovar Saída do Galpão</span> na tela do Romaneio criado.</p>
                                            <p className="mt-2 text-sm text-content-secondary">Nesse momento mágico, todos os pedidos incluídos nessa carga passam para o status "Em Trânsito", e os telefones e telas das Lojas disparam avisando que a moto acabou de sair do CD na placa informada!</p>
                                        </Step>
                                        ...
                                        <Step number="5" title="Confirmando as Coletas Externas">
                                            ...
                                            <ul className="list-disc ml-6 mt-2 text-sm text-content-secondary space-y-2">
                                                ...
                                                <li>Acione o botão verde de <span className="font-bold text-status-success-fg bg-status-success-bg px-2 py-0.5 border border-status-success-solid/40 rounded">Coletar Item</span> ao lado daquela moto específica.</li>
                                                <li>Isso atualiza o sistema e avisa a loja compradora que a moto dela finalmente virou <span className="text-status-success-fg font-bold">Coletada</span> e começará o percurso de chegada!</li>
                                            </ul>
                                        </Step>
                                        ...
                                        <Step number="6" title="Finalização Fica Por Conta das Lojas">
                                            ...
                                            <p className="text-sm text-content-secondary mt-2"><strong>Atenção:</strong> A loja só consegue finalizar o pedido quando 100% das motos forem despachadas pelo CD. Se o CD enviou parcialmente, o botão Finalizar ficará bloqueado até a entrega total.</p>
                                        </Step>
                                    </div>

                                    {/* CONTROLE DE ESTOQUE CD */}
                                    <div className="bg-status-success-bg p-6 rounded-2xl border border-status-success-solid/20 mt-8 mb-8">
                                        <h3 className="text-xl font-black text-status-success-fg mb-4 flex items-center gap-2">
                                            <ArchiveBoxIcon className="w-6 h-6" /> Controle de Estoque (Visão CD)
                                        </h3>
                                        <p className="text-sm text-status-success-fg mb-4">Acesso direto pelo menu "Estoque" (Motos) no painel esquerdo.</p>
                            ...
                            {activeTab === 'drive' && (
                                <div className="space-y-10 animate-fade-in">
                                    <HeaderSection 
                                        ...
                                        color="green"
                                    />
                                    <div className="bg-surface-card p-6 rounded-2xl border border-line shadow-sm space-y-6">
                                        <p className="font-medium text-content-secondary">Ao clicar em Conferir e Finalizar, os arquivos fotográficos são automaticamente convertidos e salvos na Nuvem.</p>
                                        
                                        <div className="mt-4 bg-surface-sunken border border-line rounded p-4">
                                            <h4 className="font-bold text-content-primary mb-2">Erros Comuns no Envio de Foto (Celular):</h4>
                                            <ul className="list-disc ml-5 space-y-2 text-sm text-content-secondary">
                                ...
                                <div className="space-y-8 animate-fade-in">
                                    ...
                                    <div className="grid gap-4">
                                        ...
                                        <FaqItem question="O que significam as cores do Calendário?">
                                            <strong className="text-status-warning-fg">🟡 Amarelo = Agendado (Pré-reserva)</strong> — O CD reservou a data, mas não confirmou. Pode mudar.<br/>
                                            <strong className="text-status-success-fg">🟢 Verde = Confirmado</strong> — Viagem oficializada. Os pedidos mudam para "Rota Confirmada".<br/>
                                            <strong className="text-status-danger-fg">🔴 Vermelho = Cancelado/Expirado</strong> — A rota foi desfeita ou a data venceu.
                                        </FaqItem>
                                    </div>
                                    ...
                                    <div className="mt-10 pt-10 border-t border-line">
                                        <div className="bg-surface-inverted text-white p-8 rounded-2xl shadow-xl flex flex-col md:flex-row justify-between items-center gap-8">
                                            ...
                                            <div className="text-center md:text-left">
                                                ...
                                                <p className="text-content-muted mt-2 text-sm">Problemas técnicos, senhas ou erros.</p>
                                                
                                                <div className="mt-4 space-y-2">
                                                    <div className="flex items-center gap-3 justify-center md:justify-start bg-surface-inverted px-4 py-2 rounded-lg">
                                                        <PhoneIcon className="w-6 h-6 text-status-success-solid" />
                                                        <div className="text-left">
                                                            <p className="text-[10px] text-content-muted uppercase">WhatsApp / Plantão</p>
                                                            <p className="font-mono text-lg font-bold text-status-success-solid">(91) 98492-8535</p>
                                                        </div>
                                                    </div>
                                                    
                                                    <div className="flex items-center gap-3 justify-center md:justify-start bg-surface-inverted px-4 py-2 rounded-lg">
                                                        <EnvelopeIcon className="w-6 h-6 text-status-info-bg" />
                                                        <div className="text-left">
                                                            <p className="text-[10px] text-content-muted uppercase">E-mail Corporativo</p>
                                                            <p className="font-mono text-sm font-bold text-status-info-bg">ti@shineraybysabel.com.br</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="text-center md:text-right border-t md:border-t-0 md:border-l border-line-strong pt-6 md:pt-0 md:pl-8">
                                                <p className="text-[10px] uppercase tracking-widest text-content-muted font-bold mb-2">Desenvolvimento & Arquitetura</p>
                                                <h5 className="text-2xl font-black text-white tracking-tight">Délcio Farias Dias Neto</h5>
                                                <p className="text-content-muted text-xs mt-1">Full Stack Developer • Shineray System V3</p>
                                                <div className="mt-4 inline-flex items-center gap-2 bg-surface-inverted px-3 py-1 rounded-full text-[10px] text-content-muted border border-line-strong">
...
function HeaderSection({ title, subtitle, color }) {
    const colors = {
        red: 'text-status-danger-fg border-status-danger-solid',
        blue: 'text-status-info-fg border-status-info-solid',
        green: 'text-status-success-fg border-status-success-solid',
        gray: 'text-content-secondary border-line-strong',
        purple: 'text-brand-700 border-brand-500',
    };
    ...
    return (
        <div className={`border-l-4 ${colors[color].split(' ')[1]} pl-6 mb-8`}>
            ...
            <p className="text-content-muted text-lg mt-1">{subtitle}</p>
        </div>
    );
}
    ...
    return (
        <div className="flex gap-4 md:gap-6 group">
            <div className="flex-shrink-0">
                <div className="w-12 h-12 rounded-full bg-surface-inverted text-white flex items-center justify-center font-bold text-xl shadow-lg group-hover:bg-surface-inverted transition-colors duration-300 ring-4 ring-line">
                    {number}
                </div>
                <div className="h-full w-0.5 bg-surface-sunken mx-auto my-2 group-last:hidden"></div>
            </div>
            <div className="flex-1 pb-8 border-b border-line last:border-0">
                <h4 className="text-xl font-bold text-content-primary mb-3">{title}</h4>
...
function FaqItem({ question, children }) {
    return (
        <details className="group bg-surface-card border border-line rounded-lg overflow-hidden transition-all duration-300 open:shadow-md open:border-line-strong">
            <summary className="font-bold text-content-secondary p-5 cursor-pointer flex items-center justify-between hover:bg-surface-sunken select-none transition-colors">
                <div className="flex items-center gap-3">
                    <span className="text-content-muted bg-surface-sunken p-1.5 rounded text-xs"><QuestionMarkCircleIcon className="w-4 h-4" /></span> 
                    {question}
                </div>
                <span className="text-content-muted group-open:rotate-180 transition-transform transform duration-300">▼</span>
            </summary>
            <div className="p-5 pt-0 text-content-secondary text-sm ml-10 leading-relaxed border-t border-transparent group-open:border-line animate-fade-in">
                {children}
            </div>
        </details>
    );
